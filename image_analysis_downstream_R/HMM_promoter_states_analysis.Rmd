...
plot_hists <- function(df){
  ...
  # all timepoints
  #g0<- ggplot(df, aes(x=tbet)) + geom_histogram(bins=1000) + xlim(0, 1.5e7) + ggtitle("tbet")
  #g1<- ggplot(df, aes(x=tbet.deriv)) + geom_histogram(bins=1000) + xlim(-1e4, 1e6) + ggtitle("tbet.deriv")
  g2<- ggplot(df, aes(x=tbet.deriv.log10_new)) + geom_histogram(binwidth=0.1)  + ggtitle("tbet.deriv.log10")
  # ggarrange(g0, g1, g2, ncol=2, nrow=2)

    # all timepoints
  g3<- ggplot(df, aes(x=tcf1)) + geom_histogram(bins=1000)  + ggtitle("tcf1")
  g4<- ggplot(df, aes(x=tcf1.deriv)) + geom_histogram(bins=1000) + ggtitle("tcf1.deriv")
  g5<- ggplot(df, aes(x=tcf1.deriv.log10_new)) + geom_histogram(binwidth=0.1) + scale_x_continuous(breaks = scales::pretty_breaks(n = 20)) + xlim(2,5.5)+ ggtitle("tcf1.deriv.log10")
  
  
  ggarrange(g3, g4, g5, ncol=3, nrow=1)
}
...
# if channel is tbet
if (channel == "tbet"){ 
    hist <- ggplot(df, aes(x=tbet.deriv.log10_new, fill= cond))  + geom_histogram(binwidth=.05, alpha=.4, position= 'identity') + xlim(3,6.5) + labs(x = "log synthesis rate") +
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), plot.background = element_blank(), axis.line = element_line(colour = "black"), text=element_text(size=16) , legend.title=element_blank())+ ggtitle(title) +
    scale_fill_manual(values=c("cyan3", "darkcyan")) 
    
      dens <- ggplot(df, aes(x=tbet.deriv.log10_new, fill= cond))  + geom_density(binwidth=.05, alpha=.4) + xlim(3,6.5) + xlab("log.tbet.deriv") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), plot.background = element_blank(), axis.line = element_line(colour = "black"), text=element_text(size=16) , legend.title=element_blank())+ ggtitle(title) +
    scale_fill_manual(values=c("cyan3", "darkcyan"))
}

# if channel is tcf1
else if (channel == "tcf1"){
...
# if running in single file
# filename <- "~/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/Matlab_Imaging_Transfer/20200129_CD8_KL_infl/R_input/tracks_trlong_only_0IL12/pos 10_trlong.mat"
...
# all_pos_df <- log_deriv(all_pos_df, 1)
...
# Now augment all_pos_df (trlong data) with infromation from trlong_trs 
```{r}
...
```{r}
saveRDS(all_pos_df, file ="/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/all_pos_df_102721")
```

# Load all pos
```{r}
all_pos_df <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/all_pos_df_102721")
```
...
# TCF1
# without filtering
plot_overlay("tcf1", all_pos_df, 0, 150, "intensity")[[1]]
plot_overlay("tcf1", all_pos_df, 0, 150, "intensity")[[2]]
plot_overlay("tcf1", all_pos_df, 0, 150, "deriv")[[1]]
plot_overlay("tcf1", all_pos_df, 0, 150, "deriv")[[2]]
plot_overlay("tcf1", all_pos_df, 0, 150, "log deriv")[[1]]
plot_overlay("tcf1", all_pos_df, 0, 150, "log deriv")[[2]]

# with filtering
plot_overlay("tcf1", all_pos_df_distinct, 0, 150, "intensity")[[1]]
plot_overlay("tcf1", all_pos_df_distinct, 0, 150, "intensity")[[2]]
plot_overlay("tcf1", all_pos_df_distinct, 0, 150, "deriv")[[1]]
plot_overlay("tcf1", all_pos_df_distinct, 0, 150, "deriv")[[2]]
plot_overlay("tcf1", all_pos_df_distinct, 0, 150, "log deriv")[[1]]
...
plot_overlay("tcf1", all_pos_df_distinct, 0, 10,"deriv")[[2]]

plot_overlay("tcf1", all_pos_df_distinct, 20, 40,"deriv")[[1]]
plot_overlay("tcf1", all_pos_df_distinct,20, 40,"deriv")[[2]]

plot_overlay("tcf1", all_pos_df_distinct, 40, 60,"deriv")[[1]]
plot_overlay("tcf1", all_pos_df_distinct,40, 60,"deriv")[[2]]

plot_overlay("tcf1", all_pos_df_distinct, 60, 80,"deriv")[[1]]
plot_overlay("tcf1", all_pos_df_distinct,60, 80,"deriv")[[2]]

plot_overlay("tcf1", all_pos_df_distinct, 80, 150,"deriv")[[1]]
plot_overlay("tcf1", all_pos_df_distinct,80, 150,"deriv")[[2]]
...
# TCF1
# model 3 -- (Model 1 in suppl. figure)
# 4 state (linear):
num_states <- 4
data_channel <- "tcf1"
...
# # model 2 -- (Model 2 in suppl. figure)
# #6/24 - 4 states
# # TCF1
# # 4 state (linear):
# num_states <- 4
...
# # model 4 (Model 3 in suppl. figure)
# #6/24 - 3 states
# # TCF1
# # 3 state (linear):
# num_states <- 3
...
# # model 5 (Model 4 in suppl. figure)
# #6/24 - 3 states
# # TCF1
# # 3 state (linear):
# num_states <- 3
...
#save fits (msm object) with model number
#saveRDS(fit, file ="/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model3_102721")
# saveRDS(fit, file ="/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model2_102721")
# saveRDS(fit, file ="/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model4_102721")
# saveRDS(fit, file ="/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model5_102721")

# compare 102721 fits
fit_m3 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model3_102721")
fit_m2 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model2_102721")
fit_m4 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model4_102721")
fit_m5 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model5_102721")
...
# load model
fit_tcf1_m3 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/Tcf1_model3_102721")

# models to run
fit_tcf1 <- fit_tcf1_m3
...
# saveRDS(pred_0IL12, file = "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_0IL12_102721_tcf1-m3")
# saveRDS(pred_1IL12, file = "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_1IL12_102721_tcf1-m3")
...
Grouping by lineage, find the run lengths that are 32 time points are less. Confirm that these short runs are actually =< 8 hrs by checking t column. If yes, convert these states to the previous state. I will first filter out the shorters runs first until there are no runs remaining =< 8 hrs.
...
pred_0IL12 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_0IL12_102721_tcf1-m3")
pred_1IL12 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_1IL12_102721_tcf1-m3")
...
saveRDS(pred_0IL12, file = "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_0IL12_102721_tcf1-m3_filtered")
saveRDS(pred_1IL12, file = "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_1IL12_102721_tcf1-m3_filtered")
...
- For each state, sum the total counts of promoter activities classified as that state. Take this as an approximation for AUC, e.g. height of gaussian for now.
- Plot theoretical gaussians on promoter activity histograms 

```{r}

output_folder_fit <- "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/goodness_fit_102721"
...
plot_theor_expt_fits <- function(cond, df, fit_col, state_col, state, mu, sigma){
  ...
  gg_theme <- theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), plot.background = element_blank(), axis.line = element_line(colour = "black"), text=element_text(size=20), axis.text = element_text(size=24)) 
  ...
  png(file.path(output_folder_fit, paste("residuals_state", state, ".png", sep="")), width = 300, height = 300)
  par(pty="s")
  # qqnorm(res, main = paste("State ", state, ", Residuals", sep=""), cex.axis = 2, cex.lab = 2)
  qqnorm(res, main ="", ylab="", xlab="", cex.axis = 1.5, cex.lab = 1.5)
  qqline(res)
  dev.off()
  
  

  return(list(g1, g2, sum_in_state))
}

####
pred_0IL12 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_0IL12_102721_tcf1-m3_filtered")
pred_1IL12 <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/pred_1IL12_102721_tcf1-m3_filtered")
...
s1_fit <- plot_theor_expt_fits(cond, df, fit_col, state_col, 1, -15042.79, 11007.98)
png(file.path(output_folder_fit, paste("dist_state", 1, ".png", sep="")), width = 300, height = 300)
s1_fit[[1]]
dev.off()
png(file.path(output_folder_fit, paste("quantiles_state", 1, ".png", sep="")), width = 300, height = 300)
s1_fit[[2]]
dev.off()

s2_fit <- plot_theor_expt_fits(cond, df, fit_col, state_col, 2,  5544.265, 8286.387)
png(file.path(output_folder_fit, paste("dist_state", 2, ".png", sep="")), width = 300, height = 300)
s2_fit[[1]]
dev.off()
png(file.path(output_folder_fit, paste("quantiles_state", 2, ".png", sep="")), width = 300, height = 300)
s2_fit[[2]]
dev.off()

s3_fit <- plot_theor_expt_fits(cond, df, fit_col, state_col, 3, 27665.118, 6729.124)
png(file.path(output_folder_fit, paste("dist_state", 3, ".png", sep="")), width = 300, height = 300)
s3_fit[[1]]
dev.off()
png(file.path(output_folder_fit, paste("quantiles_state", 3, ".png", sep="")), width = 300, height = 300)
s3_fit[[2]]
dev.off()

s4_fit <- plot_theor_expt_fits(cond, df, fit_col, state_col, 4, 64391.54, 28295.92)
png(file.path(output_folder_fit, paste("dist_state", 4, ".png", sep="")), width = 300, height = 300)
s4_fit[[1]]
dev.off()
png(file.path(output_folder_fit, paste("quantiles_state", 4, ".png", sep="")), width = 300, height = 300)
s4_fit[[2]]
dev.off()
...
# Plot HMM results - filtered -- may remove
For now, plot TCF1 promoter states only
(include Tbet code commented out)
Include new TCF1 2 on states 
Normalize across all tracks plotted
Include capabiliy to plot only a subset of positions or conditions
```{r}
...
Can highlight specific tracks if desired 
```{r}
...
# df_pred <- df_pred %>% filter(pos=="pos 91", lin=="2", longtrack %in% c(1,2,3))
# df_pred <- df_pred %>% filter(pos=="pos 91", lin=="2", longtrack %in% c(4,6,7))
# df_pred <- df_pred %>% filter(pos=="pos 91", lin=="2", longtrack %in% c(8,9,10,11))
# df_pred <- df_pred %>% filter(pos=="pos 91", lin=="2", longtrack %in% c(12,13,14,15,16))
# df_pred <- df_pred %>% filter(pos=="pos 93", lin=="1")
...
p_all <- ggplot() + 
  ...
  # geom_line(data = df_pred, mapping = aes(x = t, y = tcf1.smooth, group = ID), alpha = .2) +
  # geom_line(data = filter(df_pred, ID ==eff_ID), mapping = aes(x = t, y = tcf1.smooth, group = ID), color = eff_col, alpha = 1, size = 2) +
  # geom_line(data = filter(df_pred, ID ==mem_ID), mapping = aes(x = t, y = tcf1.smooth, group = ID), color = mem_col, alpha = 1, size = 2) +
  scale_color_manual(name = "tcf1_fit_filtered",
                       values = state_colors) +
  #first divtime
...
# save_plot(p_all, filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/longtracks_color-state/pos93_lin1_all.pdf', base_asp = 1)
save_plot(p_all, filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/longtracks_color-state/pos91_lin2_all.pdf', base_asp = 1)
```
...
# Plot longtracks overlaid on single plot 
Add capability to group tracks by different features
```{r}
state_colors = c("1" = "#000004FF", # viridis::inferno 5 color
                                    "2" = "#56106EFF",
                                    "3" = "#BB3754FF",
...
# view
#num <- nrow(distinct(df_pred, pos, lin))
num <- nrow(distinct(df_pred, cond))
for (p in seq(from=1, to=num)){
  print(plots$plot[[p]])
}

save_plot(plots$plot[[1]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_0IL12.png', base_asp = 1)
save_plot(plots$plot[[2]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_1IL12.png', base_asp = 1)

```

# Plot longtracks overlaid on single plot - CD69 or area
Add capability to group tracks by different features
```{r}
state_colors = c("1" = "#000004FF", # viridis::inferno 5 color
                                    "2" = "#56106EFF",
  ...
  do(
    ...
    geom_line( alpha = 0.2) +
            scale_color_manual(name = "tcf1_fit_filtered",
                     values = state_colors) +
      ylim(0,3000) +
      # labs(x = "Time (hrs)", y = "Intensity (a.u.)")  +
      # labs(x = "Time (hrs)", y = expression(Area~( px^{"2"})))  +
      labs(x = "", y = "")  +
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), plot.background = element_blank(), axis.ticks.y=element_blank(),  axis.line = element_line(colour = "black"), text=element_text(size=24), legend.position = "none")
    p
  })
...
# area
# save_plot(plots$plot[[1]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_0IL12_area.png', base_asp = 1.2)
# save_plot(plots$plot[[2]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_1IL12_area.png', base_asp = 1.2)

# cd69
save_plot(plots$plot[[1]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_0IL12_cd69.png', base_asp = 1.2)
save_plot(plots$plot[[2]], filename = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/track_overlay_states_1IL12_cd69.png', base_asp = 1.2)
...
saveRDS(df_pred, file = "/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/df_pred_102721_tcf1-m3_filtered_added-stats.RDS")
...
## Plot
bw = 4 # binwidth
output_folder = '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721'
...
# Plot promoter activities in each state, for both +/- IL-12 
```{r}

bw = 1
# xlabel = expression(Derivative~(x ~10^{"3"}~a.u.))
xlabel = ""
ylabel = ""
order  = c("1IL12", "0IL12")

g_state1 <- rbind(pred_0IL12, pred_1IL12) %>% 
  filter(tcf1_fit_filtered == 1) %>%
  mutate(cond = factor(cond, levels = order)) %>%
    ggplot(aes(x=tcf1.deriv/1000, fill = cond)) +
  geom_histogram(alpha = 0.6, position = 'identity', binwidth = bw)+
  facet_grid(cond ~ ., scales = "fixed")+
  labs(x=xlabel, y=ylabel)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), plot.background = element_blank(),axis.line = element_line(colour = "black"), text=element_text(size=16))

g_state2 <- rbind(pred_0IL12, pred_1IL12) %>% 
  filter(tcf1_fit_filtered == 2) %>%
  mutate(cond = factor(cond, levels = order)) %>%
    ggplot(aes(x=tcf1.deriv/1000, fill = cond)) +
  geom_histogram(alpha = 0.6, position = 'identity', binwidth =  bw)+
  facet_grid(cond ~ ., scales = "fixed")+
...
g_state3 <- rbind(pred_0IL12, pred_1IL12) %>% 
  filter(tcf1_fit_filtered == 3) %>%
  mutate(cond = factor(cond, levels = order)) %>%
    ggplot(aes(x=tcf1.deriv/1000, fill = cond)) +
  geom_histogram(alpha = 0.6, position = 'identity', binwidth = bw)+
  facet_grid(cond ~ ., scales = "fixed")+
...
g_state4 <- rbind(pred_0IL12, pred_1IL12) %>% 
  #distinct(ID, .keep_all = TRUE) %>%
  filter(tcf1_fit_filtered == 4) %>%
  #filter(cond == '0IL12') %>%
  # ggplot(aes(x=tcf1.deriv, fill = cond)) +
  mutate(cond = factor(cond, levels = order)) %>%
    ggplot(aes(x=tcf1.deriv/1000, fill = cond)) +
  geom_histogram(alpha = 0.6, position = 'identity', binwidth =  bw)+
  facet_grid(cond ~ ., scales = "fixed")+
...
# save
save_plot(g_state1, filename = file.path(output_folder, "activity_hist_state1.pdf"), base_asp = 1.2)
save_plot(g_state2, filename = file.path(output_folder, "activity_hist_state2.pdf"), base_asp = 1.2)
save_plot(g_state3, filename = file.path(output_folder, "activity_hist_state3.pdf"), base_asp = 1.2)
save_plot(g_state4, filename = file.path(output_folder, "activity_hist_state4.pdf"), base_asp = 1.2)
```
...
df_pred <- readRDS("/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/6.Image_analysis/HMM_promoter_states/df_pred_102721_tcf1-m3_filtered_added-stats.RDS")
# df_pred <- df_pred %>% filter(tr_cell_cycle_time >= 5) # remove rows with cell cycle less than 5 hr
...
## Downstream analysis of predicted states
# Functions 
```{r}
################################################################################################################################################
...
g_HI <- df_summary_comb %>% 
  ggplot(aes(x=t_theor, y = tcf1_frac_S4, color = factor(cond, levels = c("1IL12","0IL12")))) + 
  geom_point() + 
  xlab('Time (hr)') + ylab("Fraction HI ON") +
  gg_theme 

g_LO <- df_summary_comb %>% 
  ggplot(aes(x=t_theor, y = tcf1_frac_S3, color = factor(cond, levels = c("1IL12","0IL12")))) + 
  geom_point() + 
  xlab('Time (hr)') + ylab("Fraction LOW ON") +
  gg_theme 

g_SIL <- df_summary_comb %>% 
  ggplot(aes(x=t_theor, y = tcf1_frac_S2, color = factor(cond, levels = c("1IL12","0IL12")))) + 
  geom_point() + 
  xlab('Time (hr)') + ylab("Fraction silenced") +
  gg_theme 
...
Version 2: here I am incorporating promoter states beyond a single cell cycle and also quantifying re-activation
```{r}
...
```{r}
output_folder_sister <- '/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/sisters_filter_neg'
...
```{r}
pc_states_old <- readRDS('/Users/kathleenabadie/Google Drive/1.Lab_starting_March_2018/5.Experiments_labwork/Analysis/20200129_CD8_KL_infl/R_output_figures_102721/sisters/pc_states.RDS')
pc_states_new <- readRDS(file.path(output_folder_sister, 'pc_states.RDS'))
...
# filter by gen
pc_states <- pc_states %>% filter(gen == 5)
gen <- '5'
...
# summary$gen <- '1'
# summary_gen1 <- summary

# summary$gen <- '2'
# summary_gen2 <- summary

# summary$gen <- '3'
# summary_gen3 <- summary

# summary$gen <- '4'
# summary_gen4 <- summary


summary$gen <- '5'
summary_gen5 <- summary

plot_events(summary, gg_theme, output_folder_sister, gen )
...
```{r}
# plot_longtracks_pairs <- function(df, position, child_1, child_2, divtime_highlight, state_colors, theme, t_lim=100){
# plot_longtracks_pairs <- function(x, df, state_colors, theme, t_lim=100){
...
test <- ON.2xOFF %>% dplyr::select(pos, child_1, child_2, child_1_ID, child_2_ID, t) %>%  pmap_chr(plot_longtracks_pairs, df_pred, state_colors=state_colors, theme = gg_theme, colors = state_colors, subfolder= "p2/", t_lim=100)
```

```{r}
# ON.OFF_ON
ON.OFF_ON$t <-ON.OFF_ON$max_t
test <- ON.OFF_ON %>% dplyr::select(pos, child_1, child_2, child_1_ID, child_2_ID, t) %>%  pmap_chr(plot_longtracks_pairs, df=rbind(pred_0IL12, pred_1IL12), state_colors=state_colors, theme = gg_theme, colors = state_colors, subfolder= "p1/",t_lim=100)
```

```{r}
# ON.2xON
ON.2xON$t <-ON.2xON$max_t
test <- ON.2xON %>% dplyr::select(pos, child_1, child_2, child_1_ID, child_2_ID, t) %>%  pmap_chr(plot_longtracks_pairs, df=rbind(pred_0IL12, pred_1IL12), state_colors=state_colors, theme = gg_theme, colors = state_colors, subfolder= "p0/", t_lim=100)
