...
% Inflexible (late) decision making model
% ----------------------------------------
% x - naive cells, cells/uL
% y - memory cells, cells/uL
% z - effector cells, cells/uL
% v - viral load,
% m - inactive innate immune cell: cells/uL
% n - activated innate immune cell: cells/uL
...
params.ay = 0.25;  % maximal rate of memory to effector differentiation
...
params.bz = 0.0008;  % maximum rate of effector de-differentiation
...
examples = [12 46 82];   % indices for scanned parameter for plotting;
...
semilogy([scan(1) scan(end)],[predictedfrac predictedfrac]);
...
title('obligate epigenetic switch')
...
semilogy([scan(examples(1)), scan(examples(1))], [10^(-3) 10^(3)],'k');
semilogy([scan(examples(2)), scan(examples(2))], [10^(-3) 10^(3)],'k');
semilogy([scan(examples(3)), scan(examples(3))], [10^(-3) 10^(3)],'k');
...
f(1) = -ax*v*x     ; %x - naive
f(2) = gy*v/(v+Ky1)*y - ay*v/(v+Ky2)*y + bz/(v/Kz2+1)*z; %y - memory
f(3) = ax*v*x + gz*v/(v+Kz1)*z + ay*v/(v+Ky2)*y - bz/(v/Kz2+1)*z - dz*z ; %z - effector
